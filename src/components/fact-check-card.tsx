...
export function FactCheckCard({
  ...
  return (
    <article className="surface-card group flex h-full flex-col p-5 transition duration-200 hover:-translate-y-0.5 hover:border-[#b9cbd2] hover:shadow-[0_15px_40px_rgb(19_43_58/7%)] sm:p-6">
      ...
      <blockquote className={`mt-4 font-extrabold tracking-[-0.02em] text-ink ${compact ? "line-clamp-2 text-[15px] leading-6" : "text-base leading-7"}`}>
        “{statement}”
      </blockquote>
      <p className="mt-2 text-xs font-semibold text-muted">{influencer}</p>
      ...
      {!compact && <p className="mt-4 text-sm leading-6 text-slate-600">{summary}</p>}
      ...
      <div className="mt-auto pt-5">
        ...
        <Link
          ...
          className="mt-4 flex min-h-11 items-center justify-between rounded-xl bg-slate-50 px-4 text-sm font-extrabold text-ink transition group-hover:bg-[#eaf3f3] group-hover:text-brand"
        >
          상세 근거 보기
